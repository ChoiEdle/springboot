...
public class JdbcTemplateOrderRepository implements OrderRepository{
    ...
    public int saveOrders(KakaoPay kakaoPay) {
        String sql = """
                insert into orders(order_code, member_id, shipping_fee, discount_amount, total_amount, receiver_name, 
                receiver_phone, zipcode, address1, address2, memo, odate)
                values(?,?,?,?,?,?,?,?,?,?,?,now())
                """;
        Object[] params = {
                kakaoPay.getOrderId(), kakaoPay.getUserId(), kakaoPay.getPaymentInfo().getShoppingFee(),
                kakaoPay.getPaymentInfo().getDiscountAmount(), kakaoPay.getTotalAmount(), kakaoPay.getReceiver().getName(),
                kakaoPay.getReceiver().getPhone(), kakaoPay.getReceiver().getZipcode(), kakaoPay.getReceiver().getAddress1(),
                kakaoPay.getReceiver().getAddress2(), kakaoPay.getReceiver().getMemo()
        };
    ...
    public int saveOrderDetail(KakaoPay kakaoPay) {
        String sql = """
            ...
            select :orderCode, pid, name as pname, size, qty, totalPrice as pid_total_price, :discount       
            from view_cartList where cid in (:cidList)
        """;
