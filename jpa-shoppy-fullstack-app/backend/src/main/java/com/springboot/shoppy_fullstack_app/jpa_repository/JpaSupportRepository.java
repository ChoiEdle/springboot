...
public interface JpaSupportRepository extends JpaRepository<Support, Integer> {
    ...
    @Query("""
            select s from Support s where s.stype = :stype and (
            (:type = 'title' and s.title like %:keyword%) or 
            (:type = 'content' and s.content like %:keyword%) or
            (:type = 'cdate' and s.rdate like %:keyword%)
            )""")
    Page<Support> search(@Param("type") String type, @Param("keyword") String keyword, @Param("stype") String stype, Pageable pageable);

    @Query("""
            select s from Support s where 
            (:type = 'title' and s.title like %:keyword%) or 
            (:type = 'content' and s.content like %:keyword%) or
            (:type = 'cdate' and s.rdate like %:keyword%)
            """)
    Page<Support> search(@Param("type") String type, @Param("keyword") String keyword, Pageable pageable);
