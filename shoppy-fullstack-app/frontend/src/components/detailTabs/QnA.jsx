                ...
                <tbody>
                    {qnaData && qnaData.map(item =>
                        <tr style={{paddingTop:"10px"}}>
                            <td style={{width:"10%"}}>
                                {item.complete && item.complete ? <span>답변완료</span> : <span>접수완료</span>}
                            </td>
                            <td style={{width:"60%"}}>
                                <span style={{cursor:"pointer"}} onClick={()=>{handleToggle(item.qid && item.qid)}}>{item.title && item.title}</span>
                                {item.lock && item.lock ? <CiLock /> : ""}
                                {item.qid && openQid === item.qid && <span>{item.content && item.content}</span>}
                            </td>
                            <td style={{width:"15%"}}>
                                {item.id && item.id}
                            </td>
